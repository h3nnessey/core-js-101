...
function getFactorial(n) {
  return Array(n).fill(0).map((_, i) => i + 1).reduce((acc, curr) => acc * curr, 1);
}
...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1).fill(null).map((_, i) => n1 + i).reduce((acc, curr) => (acc + curr), 0);
}
 ...
 * Each rectangle represented by object
 *  {
 *     top: 5,
 *     left: 5,
 *     width: 20,
...
function doRectanglesOverlap(rect1, rect2) {
  const isLeftSideOverseas = (rect2.left > rect1.left + rect1.width);
  const isTopSideOverseas = (rect2.top > rect1.top + rect1.height);

  return !(isLeftSideOverseas || isTopSideOverseas);
}
